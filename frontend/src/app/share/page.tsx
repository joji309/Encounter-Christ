...
import { Share2, Copy, Check, MessageCircle, Sparkles, ArrowRight, Heart } from 'lucide-react';
import { SITE_URL } from '@/lib/site-url';
import ImageCardComposer from '@/components/ImageCardComposer';
...
// ── Per-card component so each card can own its own composer open/close state
function ShareCard({
  snippet,
  idx,
  copiedIndex,
  ...
  return (
    <>
      <div className="sacred-glass-card rounded-3xl p-6 flex flex-col justify-between space-y-4 border border-amber-300 bg-white shadow-md">
        <div>
        ...
        <div className="space-y-2 pt-2 border-t border-amber-200">
          {/* Row 1: WhatsApp text + Copy */}
          <div className="flex items-center gap-2">
            <button
              onClick={() => onWhatsApp(snippet.text)}
            ...
            <button
              onClick={() => onCopy(snippet.text, idx)}
            ...
            >
              {copiedIndex === idx ? (
                <><Check className="w-3.5 h-3.5 text-amber-700" /><span className="text-amber-800 font-bold">Copied</span></>
              ) : (
                <><Copy className="w-3.5 h-3.5 text-amber-700" /><span>Copy</span></>
              )}
            </button>
          </div>

          {/* Row 2: Image Card Creator */}
          <button
            onClick={() => setComposerOpen(true)}
            className="w-full py-2.5 px-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 active:scale-95 text-white text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-sm hover:shadow-amber-400/40 group"
          >
            <span className="text-sm leading-none">🎨</span>
            <span>Create Image Card for WhatsApp</span>
            <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
          </button>
        </div>
      </div>

      {/* Image Card Composer Modal */}
      <ImageCardComposer
        isOpen={composerOpen}
...
export default function ShareJesusPage() {
  ...
  const shareableSnippets: SnippetType[] = [
    {
    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto space-y-16">
      ...
      <div className="space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-stone-600 font-medium pl-7">
            Send a ready-made message — or tap{' '}
            <strong className="text-amber-800">🎨 Create Image Card</strong> to design a beautiful
            image with your own photo and share it on WhatsApp.
          </p>
        </div>
      ...
      <div className="sacred-glass-card rounded-3xl p-8 sm:p-12 border border-amber-300 bg-gradient-to-br from-amber-50/90 via-white to-amber-100/70 space-y-4 text-center shadow-xl">
        ...
        <h3 className="font-serif text-2xl font-bold text-stone-900">
          Inspired by Blessed Carlo Acutis (1991&ndash;2006)
        </h3>
        <p className="text-xs sm:text-sm text-stone-700 max-w-2xl mx-auto leading-relaxed font-medium">
          At age 11, Blessed Carlo Acutis used modern computer programming to build the world&apos;s
          first virtual museum of Eucharistic Miracles to bring people back to the Catholic faith.
          He famously stated:
        </p>
        <blockquote className="font-serif italic text-base text-amber-900 max-w-xl mx-auto font-bold">
          &ldquo;The Eucharist is my highway to Heaven. All people are born originals, but many end
          up as photocopies.&rdquo;
        </blockquote>
      </div>
    </div>
  );
